...
static constexpr unsigned long StopBundleDisabled =
    std::numeric_limits<unsigned long>::max();
static cl::opt<unsigned long>
    StopBundle("sbvec-topdown-stop-bndl", cl::init(StopBundleDisabled), cl::Hidden,
...
Action *TopDownVec::vectorizeRec(ArrayRef<Value *> Bndl, unsigned Depth,
  ...
  LLVM_DEBUG(dbgs() << DEBUG_PREFIX << "canVectorize() Bundle:\n";
             VecUtils::dump(Bndl));
  const auto &LegalityRes = Legality.canVectorize(Bndl, /*SkipScheduling=*/true);
  ...
  if (LegalityRes.getSubclassID() == LegalityResultID::Widen) {
    ...
    for (Value *V : Bndl) {
      if (V->user_begin() == V->user_end()) { CanFormUserBndl = false; break; }
      UserBndl.push_back(*V->user_begin());
    }

    if (CanFormUserBndl) {
      auto *U0 = dyn_cast<Instruction>(UserBndl[0]);
      if (!U0 || IMaps->isVectorized(U0)) CanFormUserBndl = false;
      else {
        for (Value *U : drop_begin(UserBndl)) {
          auto *UI = dyn_cast<Instruction>(U);
          if (!UI || UI->getOpcode() != U0->getOpcode() || UI->getType() != U0->getType() || IMaps->isVectorized(UI)) {
...
#ifndef NDEBUG
void TopDownVec::ActionsVector::print(raw_ostream &OS) const {
  for (auto [Idx, Action] : enumerate(Actions)) {
    Action->print(OS);
    OS << "\n";
  }
}
void TopDownVec::ActionsVector::dump() const { print(dbgs()); }
#endif // NDEBUG
  ...
  for (const auto &ActionPtr : Actions) {
    ...
    case LegalityResultID::Widen: {
      ...
      case Instruction::Opcode::Store: {
        auto OpBndl = VecUtils::getOperand(Bndl, 0);
        if (Action *OpA = IMaps->getVectorForOrig(OpBndl[0])) {
          VecOperands.push_back(OpA->Vec);
        } else {
          Value *Packed = VecUtils::createPack(OpBndl, UserBB);
      ...
      default:
        ...
        for (unsigned OpIdx = 0; OpIdx < I->getNumOperands(); ++OpIdx) {
          SmallVector<Value*, 4> OpBndl = VecUtils::getOperand(Bndl, OpIdx);
          if (Action *OpA = IMaps->getVectorForOrig(OpBndl[0])) {
            VecOperands.push_back(OpA->Vec);
          } else {
            Value *Packed = VecUtils::createPack(OpBndl, UserBB);
      ...
      NewVec = VecUtils::createVectorInstr(ActionPtr->Bndl, VecOperands);
      LLVM_DEBUG(dbgs() << DEBUG_PREFIX << "New instr: " << *NewVec << "\n");
      if (NewVec != nullptr)
        VecUtils::collectPotentiallyDeadInstrs(Bndl, DeadInstrCandidates);

      VecUtils::emitUnpacksForExternalUses(ActionPtr->Bndl, NewVec, *IMaps);
      break;
    }
    ...
    case LegalityResultID::DiamondReuseWithShuffle: {
      ...
      NewVec = VecUtils::createShuffle(VecOp, Mask, UserBB);
      break;
    }
    case LegalityResultID::DiamondReuseMultiInput: {
      // Simplified for brevity, same as BottomUpVec
      const auto &Descr =
    ...
    case LegalityResultID::Pack: {
      if (Depth == 0)
        return nullptr;
      NewVec = VecUtils::createPack(Bndl, UserBB);
      break;
    }
    }
    if (NewVec != nullptr) {
...
bool TopDownVec::tryVectorize(ArrayRef<Value *> Bndl,
  ...
  LLVM_DEBUG(dbgs() << DEBUG_PREFIX << "TopDownVec: Vectorization Actions:\n";
             Actions.dump());
  emitVectors();
  VecUtils::tryEraseDeadInstrs(DeadInstrCandidates, DEBUG_PREFIX);
  return Change;
}

bool TopDownVec::runOnRegion(Region &Rgn, const Analyses &A) {
  /// FIXME: Use a different function to load seeds. getAux may
  // return non-seed instructions in the future.
